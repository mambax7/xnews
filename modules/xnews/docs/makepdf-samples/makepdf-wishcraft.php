<?php
    ...
    include_once XNEWS_MODULE_PATH . '/class/keyhighlighter.class.php';
    ...
    if ($filescount>0) {
        foreach ($filesarr as $onefile)	{
            if (strstr($onefile->getMimetype(), 'image')) {
                ...
                $newsfiles[] = array('file_id' => $onefile->getFileid(), 'visitlink' => XNEWS_MODULE_URL . '/visit.php?fileid=' . $onefile->getFileid(), 'file_realname' => $onefile->getFileRealName(), 'file_attacheddate' => formatTimestamp($onefile->getDate(), $dateformat), 'file_mimetype' => $onefile->getMimetype(), 'file_downloadname' => XNEWS_ATTACHED_FILES_URL . '/' . $onefile->getDownloadname());
            }
        }
    ...
    */
    ...
    //DNPROSSI - ADDED
    $GLOBALS['xoopsTpl']->assign('display_icons', $xnews->getConfig('displaylinkicns'));
